...
def build_run_config(type, task_id, model_dir):
    ...
    server = tf.train.Server(cluster, job_name=type, task_index=int(task_id))
    if type == "ps":
        server.join()
    return run_config
...
def build_estimator(model_dir, model_type, model_column_fn):
  ...
  run_config = build_run_config(flags.type, flags.task_id, model_dir)
...
if __name__ == '__main__':
  ...
  flags.DEFINE_string("type", "chief", "chief or worker")
  flags.DEFINE_integer("task_id", 0, " the log dir")
  if sys.argv[1] == "0":
      flags.type = "chief"
      flags.task_id = 0
  elif sys.argv[1] == "1":
      ...
      flags.type = "ps"
      flags.task_id = 0
  print("type:%s, task_id:%s" % (flags.type, flags.task_id))
  absl_app.run(main)
